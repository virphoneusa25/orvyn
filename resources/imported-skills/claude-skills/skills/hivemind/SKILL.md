---
name: hivemind
description: Orchestrate free opencode workers from ORVYN to cut token costs. Use when delegating grunt work to a single worker or a parallel swarm (scout/coder/tester) with worktree isolation, benchmarking against opencode, or when the user says "spawn a worker", "swarm", "delegate to opencode", or "/oc".
---

# Hivemind: ORVYN as Orchestrator, opencode as Free Worker Swarm

ORVYN = brain (plans, reviews, merges). opencode = disposable workers on free models
...
4. Do not execute scripts directly from this skill. bench/grader-prompt.md, bench/run-bench.mjs, oc-aggregate.mjs, oc-status.mjs, oc-worker.mjs may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
