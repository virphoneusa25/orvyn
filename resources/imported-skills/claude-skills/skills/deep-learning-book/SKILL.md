...
4. Do not execute scripts directly from this skill. capacity_planner.py, model_arithmetic.py, reading_path_planner.py, training_diagnostics.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
