...
> scripts. Works in ORVYN CLI and ORVYN web. The panel does the depth; the Judge does the call.
...
4. Do not execute scripts directly from this skill. brief_builder.py, cheapest_test_designer.py, verdict_synthesizer.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
