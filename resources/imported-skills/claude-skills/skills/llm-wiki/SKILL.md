...
├── CLAUDE.md               # Schema + conventions (ORVYN)
...
The vault's **schema** lives in CLAUDE.md (ORVYN) or AGENTS.md (Codex/Cursor/Antigravity/OpenCode). The same content works in both. This plugin ships both templates. For per-tool setup instructions see `references/cross-tool-setup.md`.
...
4. Do not execute scripts directly from this skill. append_log.py, export_marp.py, graph_analyzer.py, ingest_source.py, init_vault.py, lint_wiki.py, update_index.py, wiki_search.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
5. Omit unsupported capabilities: optional tool [claude-code; optional tool codex-cli; optional tool cursor; optional tool antigravity; optional tool opencode; optional tool gemini-cli].
