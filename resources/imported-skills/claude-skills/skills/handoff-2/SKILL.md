...
4. Do not execute scripts directly from this skill. cleanup.py, config_loader.py, handoff_self_check.py, handoff_template_generator.py, redaction_linter.py, setup.py, skill_recommender.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
