...
4. Do not execute scripts directly from this skill. forgetting_policy_linter.py, memory_architecture_picker.py, memory_cost_profiler.py, memory_density_auditor.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
