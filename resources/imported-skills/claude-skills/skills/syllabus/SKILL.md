...
> **Portability:** Requires a Consensus MCP connection, Node.js with `docx` package, and file reading capability for the syllabus. Works in ORVYN CLI natively. In ORVYN with Consensus MCP + Code Execution + file upload, the workflow is supported.
...
4. Do not execute scripts directly from this skill. citation_tracker.py, discussion_question_validator.py, generate_reading_list.js, topic_grouper.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
5. Omit unsupported capabilities: required integration Requires a Consensus MCP connection, Node.
