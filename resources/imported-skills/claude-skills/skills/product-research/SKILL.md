...
4. Do not execute scripts directly from this skill. ar_evaluator.py, config_loader.py, insight_synthesizer.py, onboard.py, saturation_planner.py, study_designer.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
