...
- **ORVYN CLI:** Native — writes markdown files directly to filesystem.
- **ORVYN web:** Works with project files / artifacts. Document the alternate path: generate files as artifacts, instruct user to save to their workspace, or use connected file system if available.
...
4. Do not execute scripts directly from this skill. kb_validator.py, section_progress_tracker.py, voice_sample_analyzer.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
