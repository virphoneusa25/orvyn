...
In **ORVYN CLI**, write the file to disk at the specified path. In **ORVYN web**, create an HTML artifact with the same content.
...
- **ORVYN CLI:** Native — writes HTML file directly to filesystem.
- **ORVYN web:** Native — produces HTML as an artifact instead of file.
...
4. Do not execute scripts directly from this skill. brand_palette_validator.py, html_validator.py, kebab_slug_generator.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
