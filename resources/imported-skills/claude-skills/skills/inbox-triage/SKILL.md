...
- **ORVYN CLI:** Native — uses Gmail / Outlook MCP, file tools for KB, web search for research.
- **ORVYN web:** Works when email MCP connector is connected (Gmail MCP available). Skill must check tool availability before assuming. If no email tool: halt with clear message.
...
4. Do not execute scripts directly from this skill. draft_safety_validator.py, kb_reader.py, search_window_calculator.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
