...
**1 — Collect.** Scans ORVYN project-history JSONL and Codex rollouts,
...
|---|---|---|
| `scripts/collect_sessions.py` | Harvest ORVYN + Codex sessions, redact secrets, sample, inventory | 0 · 3 bad input |
...
4. Do not execute scripts directly from this skill. collect_sessions.py, render_report.py, score_aggregator.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
