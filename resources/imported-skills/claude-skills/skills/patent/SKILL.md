...
> **Portability:** Requires `web_fetch` (Google Patents, Espacenet, USPTO), `WebSearch` (adjacent academic art), Node.js with `docx` package, and optionally Lens.org API key for citation-graph signals. Works in ORVYN CLI natively. In ORVYN with web tools + Code Execution + BYOK Lens.org, the workflow is supported.
...
4. Do not execute scripts directly from this skill. citation_tracker.py, family_resolver.py, sub_use_case_router.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
