...
> **Portability:** Works anywhere with outbound HTTPS — the default search lane is **free keyless APIs** (PubMed E-utilities + OpenAlex, no account, no key, no MCP). The **Consensus MCP is an optional enhancement lane** used only when connected in this session. Node.js with `docx` package is required for document generation, and (in CLI) `bash_tool`. Works in ORVYN CLI natively and in ORVYN with Code Execution.
...
2. Use these ORVYN tools when the workflow needs them: web_search, fetch_url. Do not call a tool that is not registered.
3. ORVYN has no Claude slash-command runner. Follow the workflow with the ORVYN tools named above.
4. Do not execute scripts directly from this skill. citation_tracker.py, cross_search_aggregator.py, framework_recommender.py, free_search.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
5. Omit unsupported capabilities: optional Consensus; optional optional MCP.
