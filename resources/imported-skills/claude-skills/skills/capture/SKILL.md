...
> **Connections:** No workspace accessible from here. If you're running this from ORVYN or have a project with files attached, I can fill this in. Want to share where this work lives?
...
|---|---|
| ORVYN CLI | Glob for files matching dump keywords; Grep for content matches; read top-level structure. Use `scripts/workspace_inventory.py`. |
| ORVYN with project | Check project knowledge files for thematic overlap. List file titles; surface matches by keyword. |
...
4. Do not execute scripts directly from this skill. complexity_estimator.py, dump_classifier.py, workspace_inventory.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
