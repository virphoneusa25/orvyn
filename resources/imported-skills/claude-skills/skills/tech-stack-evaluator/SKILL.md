...
4. Do not execute scripts directly from this skill. ecosystem_analyzer.py, format_detector.py, migration_analyzer.py, report_generator.py, security_assessor.py, stack_comparator.py, tco_calculator.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
