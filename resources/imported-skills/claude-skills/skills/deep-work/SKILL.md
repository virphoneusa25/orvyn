...
> **Portability:** Reasoning-led skill with 3 stdlib Python scripts. No external APIs, no LLM calls in scripts. Works in ORVYN CLI and ORVYN web. The scripts fix the arithmetic; you keep the judgment.
...
4. Do not execute scripts directly from this skill. focus_session_logger.py, shallow_work_auditor.py, time_block_planner.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
