...
> **Requires:** A browser automation environment (ORVYN CLI with computer-use, Claude Chrome Extension, or equivalent). **Skill will gracefully fail in non-automation contexts with a clear "not supported" message.**
...
2. If unavailable → **halt with clear message:** "This skill requires browser automation. Currently in {context}. Cannot proceed. Use ORVYN CLI with computer-use, Claude Chrome Extension, or equivalent."
...
4. Do not execute scripts directly from this skill. action_router.py, async_action_classifier.py, custom_prompt_template_generator.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
