...
metadata:
  version: 1.0.0
  build_pattern: "Tencent TencentDB-Agent-Memory's tiering concept rebuilt natively on ORVYN hooks; deterministic recurrence gates, no LLM, no database"
...
4. Do not execute scripts directly from this skill. memory_core.py, memory_extract.py, memory_inspect.py, memory_promote.py, validate_examples.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
