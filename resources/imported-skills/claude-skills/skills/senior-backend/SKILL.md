...
4. Do not execute scripts directly from this skill. api_load_tester.py, api_scaffolder.py, backend_decision_engine.py, database_migration_tool.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
