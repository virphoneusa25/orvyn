...
4. Do not execute scripts directly from this skill. ar_evaluator.py, config_loader.py, market_sizer.py, onboard.py, sample_size_planner.py, segmentation_scorer.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
