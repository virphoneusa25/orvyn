...
> **Portability:** Pure-reasoning skill. No external tools required. Works in ORVYN CLI + ORVYN web natively. Most portable in the v2 collection.
...
4. Do not execute scripts directly from this skill. bias_pattern_detector.py, conversation_depth_analyzer.py, directional_recommendation_validator.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
