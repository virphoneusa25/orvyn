...
> **Portability:** Requires `bash_tool` (for RePORTER POST via curl), Node.js with `docx` package, and a Consensus MCP connection. Works in ORVYN CLI natively. In ORVYN with Code Execution + Consensus MCP, the workflow is supported but slower.
...
4. Do not execute scripts directly from this skill. citation_tracker.py, fiscal_year_calculator.py, mechanism_matcher.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
