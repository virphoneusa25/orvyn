...
4. Do not execute scripts directly from this skill. ar_evaluator.py, config_loader.py, endpoint_selector.py, onboard.py, phase_gate_scorer.py, sample_size_estimator.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
