...
> in scripts. Works in ORVYN CLI and ORVYN web. The scripts do the inventory and the
...
4. Do not execute scripts directly from this skill. commitment_auditor.py, open_loop_scanner.py, weekly_review_gate.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
