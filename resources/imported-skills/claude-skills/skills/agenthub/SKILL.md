...
The main ORVYN session is the coordinator. It follows this lifecycle:
...
```bash
# Copy to your ORVYN skills directory
...
4. Do not execute scripts directly from this skill. board_manager.py, dag_analyzer.py, dry_run.py, hub_init.py, result_ranker.py, session_manager.py may run only through the terminal tool, which uses ToolGateway, PermissionEngine, and ExecutionProvider. High-risk scripts stay disabled: none.
