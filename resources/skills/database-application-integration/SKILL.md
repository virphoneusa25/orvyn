...
Connect an application to the database its config names, without inventing a schema.
...
2. Read the URL, host, and database name from config. Use PostgreSQL Operations only when the server is Postgres.
3. Do not replace the schema. Verify the application can open the connection it is configured for.
