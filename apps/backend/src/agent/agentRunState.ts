...
export interface CompletionDecisionInput {
  ...
  /** The engine runs on the user's computer: its localhost is the user's. */
  localEngine?: boolean;
}
...
function reachablePreview(events: CompletionDecisionInput["events"], localEngine = false): boolean {
  return events.some((e) => {
    if (e.type !== "preview.available") return false;
    const url = String(e.data?.url ?? "");
    return /^https?:\/\//i.test(url) && (localEngine || !/localhost|127\.0\.0\.1/i.test(url));
  });
}
...
export function decideCompletion(input: CompletionDecisionInput): CompletionDecision {
  ...
  if (!wroteSite(input.events) || !reachablePreview(input.events, input.localEngine) || !browserVerified(input.events)) return "continue";
  return "completed";
}
