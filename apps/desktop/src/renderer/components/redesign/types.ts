...
export interface MissionSummary{id:string;title:string;reason:string;meta:string;tone:MissionTone;label:string;stepsDone:number;stepsTotal:number;timeAgo:string;action?:string}
...
export interface MissionDetailData{id:string;title:string;status:string;createdAt?:string;agent?:string;project?:string;plan:MissionPlanItem[];events:{id:string;kind:string;title:string;body?:string}[];approval?:ApprovalRequest;permissions?:{label:string;value:string}[];files?:{path:string;action:string}[];deliverable?:string}
