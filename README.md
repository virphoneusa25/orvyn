...
- **Intelligence UI** — center chat/stream plus one IDE Workbench
  ...
  Artifact, and Review tabs). Browser and Preview use a sandboxed Electron
  WebContentsView — real websites render inside the Workbench. Follow ORION
  switches the active tab only. Desktop is a live Chromium session with
  Take Control / Return to ORION.
